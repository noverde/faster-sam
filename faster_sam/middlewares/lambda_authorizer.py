...
from faster_sam import web_identity_providers, cache
...
class LambdaAuthorizerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if self._cache is not None:
            redis_cache = cache.factory(self._cache)
            redis_cache.set_key(dict(request.headers))

            if redis_cache.token is None:
                content = {"message": "Unauthorized"}
                status_code = HTTPStatus.UNAUTHORIZED
                return Response(content=json.dumps(content), status_code=status_code.value)

            payload = redis_cache.get()

            if payload is None:
                payload = self.invoke_lambda(request)
                redis_cache.set(payload, 900)
