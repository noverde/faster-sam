...
class LambdaAuthorizerMiddleware(BaseHTTPMiddleware):
    ...
    def authorize_from_payload(self, payload):
        if payload is None:
            return AuthorizationStatus.ERROR
        elif payload["policyDocument"]["Statement"][0]["Effect"] == "Allow":
            return AuthorizationStatus.AUTHORIZED
        return AuthorizationStatus.UNAUTHORIZED

    def authorize_from_cache(self, request):
        token = self.get_authorization_key(dict(request.headers))

        if token is None:
            return AuthorizationStatus.UNAUTHORIZED

        authorization_value = self._cache.get(token)
        if authorization_value is not None:
            return AuthorizationStatus[authorization_value]

        payload = self.invoke_lambda(request)
        authorization = self.authorize_from_payload(payload)

        if authorization != AuthorizationStatus.ERROR:
            self._cache.set(token, authorization.value)
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if self._cache is not None:
            response = self.authorize_from_cache(request)
        else:
            payload = self.invoke_lambda(request)
            response = self.authorize_from_payload(payload)
        ...
        if response == AuthorizationStatus.ERROR:
    ...
    def get_authorization_key(self, headers: dict) -> str:
